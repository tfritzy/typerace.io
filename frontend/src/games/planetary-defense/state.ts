...
function createRelicSlots(): RelicSlot[] {
  ...
  for (let i = 0; i < RELIC_SLOT_COUNT; i++) {
    ...
    if (i % 2 === 0) {
      relic = { type: RelicType.CrystalSword, level: 1, charge: 0, remainingShots: 0, nextShotTime: 0 };
    } else if (i === 1) {
      relic = { type: RelicType.CrystalStaff, level: 1, charge: 0, remainingShots: 0, nextShotTime: 0 };
    } else if (i === 3) {
      relic = { type: RelicType.IcePotion, level: 1, charge: 0, remainingShots: 0, nextShotTime: 0 };
    }
...
function getDamageBuffMultiplier(state: GameState, slotIndex: number): number {
  ...
  for (let j = 0; j < count; j++) {
    ...
    const config: RelicTypeConfig = getRelicConfig(slot.relic.type);
