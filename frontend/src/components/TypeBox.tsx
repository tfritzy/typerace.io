...
export const TypeBox = forwardRef<TypeBoxRef, TypeBoxProps>(({ phrase, onComplete, onProgress, className, height, resetOnComplete = false, disabled = false, initialProgress = 0 }, ref) => {
  ...
  const renderText = () => {
    ...
    return chars.map((char, i) => {
      ...
      const isInCurrentWord = i >= lastCompletedWordEnd && i < input.length && isCorrect;
  ...
  return (
    <div
      className={`relative box-with-focus w-full rounded-lg px-8 py-6 cursor-text flex items-start overflow-y-auto ${hasReachedErrorLimit ? 'border-red-500!' : ''} ${disabled ? 'opacity-60' : ''} ${className || ''}`}
