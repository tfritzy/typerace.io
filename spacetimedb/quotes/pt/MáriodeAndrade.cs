using System;

namespace StdbModule;

public static partial class PortugueseQuotes
{
    public static class MáriodeAndrade
    {
        public static readonly Quote[] Quotes = new Quote[]
        {
            new Quote { Id = "https://pt.wikiquote.org/wiki/Mário_de_Andrade", Text = "Quando sinto a impulsäo lírica escrevo sem pensar tudo o que meu inconsciente me grita. Penso depois: näo só para corrigir, como para justificar o que escrevi.", Author = "Mário de Andrade" },
            new Quote { Id = "https://pt.wikiquote.org/wiki/Mário_de_Andrade", Text = "Eu sou um escritor difícil, Que a muita gente enquizila, Porém essa culpa é fácil, De se acabar duma vez: É só tirar a cortina, Que entra luz nesta escurez.", Author = "Mário de Andrade" },
            new Quote { Id = "https://pt.wikiquote.org/wiki/Mário_de_Andrade", Text = "Não devemos servir de exemplo a ninguém. Mas podemos servir de lição.", Author = "Mário de Andrade" },
            new Quote { Id = "https://pt.wikiquote.org/wiki/Mário_de_Andrade", Text = "A inspiração é fugaz, violenta. Qualquer impecilho a pertuba e mesmo emudece.", Author = "Mário de Andrade" },
            new Quote { Id = "https://pt.wikiquote.org/wiki/Mário_de_Andrade", Text = "Os verdadeiros pecados mortais talvez sejam outros.", Author = "Mário de Andrade" },
            new Quote { Id = "https://pt.wikiquote.org/wiki/Mário_de_Andrade", Text = "Durasse aquilo uma noite grande, nada mais haveria porque é engraçado como a perfeição fixa a gente.", Author = "Mário de Andrade" },
            new Quote { Id = "https://pt.wikiquote.org/wiki/Mário_de_Andrade", Text = "O famoso sorriso que quer sorrir mas está chorando, chorando muito, tudo o que a vida não chorou.", Author = "Mário de Andrade" },
            new Quote { Id = "https://pt.wikiquote.org/wiki/Mário_de_Andrade", Text = "...O soluço que engasga na risada algegre da partida, enfim livre!", Author = "Mário de Andrade" },
            new Quote { Id = "https://pt.wikiquote.org/wiki/Mário_de_Andrade", Text = "Mas por dentro era impossível saber o que havia em mim, era uma luz, uma Nossa Senhora, um gosto maltratado, cheio de desilusões claríssimas, em que eu sofria arrependido, vendo inutilizar-se no infinito dos sofrimentos humanos a minha estrela-do-mar.", Author = "Mário de Andrade" },
            new Quote { Id = "https://pt.wikiquote.org/wiki/Mário_de_Andrade", Text = "O passado é lição para se meditar, não para reproduzir.", Author = "Mário de Andrade" },
        };
    }
}