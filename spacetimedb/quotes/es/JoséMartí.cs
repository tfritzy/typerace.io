using System;

namespace StdbModule;

public static partial class SpanishQuotes
{
    public static class JoséMartí
    {
        public static readonly Quote[] Quotes = new Quote[]
        {
            new Quote { Id = "https://es.wikiquote.org/wiki/José_Martí", Text = "Ayudar al que lo necesita no sólo es parte del deber, sino de la felicidad.", Author = "José Martí" },
            new Quote { Id = "https://es.wikiquote.org/wiki/José_Martí", Text = "El alma emana, igual y eterna, de los cuerpos diversos en forma y en color.", Author = "José Martí" },
            new Quote { Id = "https://es.wikiquote.org/wiki/José_Martí", Text = "El vino, de plátano; y si sale agrio, ¡es nuestro vino!", Author = "José Martí" },
            new Quote { Id = "https://es.wikiquote.org/wiki/José_Martí", Text = "Gobernante, en un pueblo nuevo, significa creador.", Author = "José Martí" },
            new Quote { Id = "https://es.wikiquote.org/wiki/José_Martí", Text = "Hay un límite para las lágrimas que podemos derramar ante las tumbas de los muertos.", Author = "José Martí" },
            new Quote { Id = "https://es.wikiquote.org/wiki/José_Martí", Text = "La fraternidad de los desdichados es la más rápida.", Author = "José Martí" },
            new Quote { Id = "https://es.wikiquote.org/wiki/José_Martí", Text = "La palabra no está hecha para cubrir la verdad, sino para decirla.", Author = "José Martí" },
            new Quote { Id = "https://es.wikiquote.org/wiki/José_Martí", Text = "Las fronteras de nuestro espíritu son las de nuestro lenguaje.", Author = "José Martí" },
            new Quote { Id = "https://es.wikiquote.org/wiki/José_Martí", Text = "La única fuerza y la única verdad que hay en esta vida es el amor. El patriotismo no es más que amor, la amistad no es más que amor.", Author = "José Martí" },
            new Quote { Id = "https://es.wikiquote.org/wiki/José_Martí", Text = "Lo divino está en lo humano.", Author = "José Martí" },
            new Quote { Id = "https://es.wikiquote.org/wiki/José_Martí", Text = "Los bárbaros que todo lo confían a la fuerza y al violencia, nada construyen, porque sus cimientos son de odio.", Author = "José Martí" },
            new Quote { Id = "https://es.wikiquote.org/wiki/José_Martí", Text = "Los derechos se toman, no se piden; se arrancan, no se mendigan.", Author = "José Martí" },
            new Quote { Id = "https://es.wikiquote.org/wiki/José_Martí", Text = "No hay batalla entre la civilización y la barbarie, sino entre la falsa erudición y la naturaleza.", Author = "José Martí" },
            new Quote { Id = "https://es.wikiquote.org/wiki/José_Martí", Text = "No hay odio de razas, porque no hay razas.", Author = "José Martí" },
            new Quote { Id = "https://es.wikiquote.org/wiki/José_Martí", Text = "Peca contra la Humanidad el que fomente y propague la oposición y el odio de las razas.", Author = "José Martí" },
            new Quote { Id = "https://es.wikiquote.org/wiki/José_Martí", Text = "Todo hombre tiene dos madres: la naturaleza y las circunstancias.", Author = "José Martí" },
            new Quote { Id = "https://es.wikiquote.org/wiki/José_Martí", Text = "Trincheras de ideas valen más que trincheras de piedra.", Author = "José Martí" },
            new Quote { Id = "https://es.wikiquote.org/wiki/José_Martí", Text = "Un grano de poesía basta para perfumar todo un siglo.", Author = "José Martí" },
            new Quote { Id = "https://es.wikiquote.org/wiki/José_Martí", Text = "Vale más un minuto de pie que una vida de rodillas.", Author = "José Martí" },
        };
    }
}