using System;

namespace StdbModule;

public static partial class EnglishQuotes
{
    public static class AllenGinsberg
    {
        public static readonly Quote[] Quotes = new Quote[]
        {
            new Quote { Id = "https://en.wikiquote.org/wiki/Allen_Ginsberg", Text = "The war is language, language abused for Advertisement, language used like magic for power on the planet.", Author = "Allen Ginsberg" },
            new Quote { Id = "https://en.wikiquote.org/wiki/Allen_Ginsberg", Text = "The only thing that can save the world is the reclaiming of the awareness of the world. That's what poetry does. By poetry I mean the imagining of what has been lost and what can be found - the imagining of who we are and the slow realization of it.", Author = "Allen Ginsberg" },
            new Quote { Id = "https://en.wikiquote.org/wiki/Allen_Ginsberg", Text = "America I'm putting my queer shoulder to the wheel.", Author = "Allen Ginsberg" },
            new Quote { Id = "https://en.wikiquote.org/wiki/Allen_Ginsberg", Text = "America I've given you all and now I'm nothing.", Author = "Allen Ginsberg" },
            new Quote { Id = "https://en.wikiquote.org/wiki/Allen_Ginsberg", Text = "I smoke marijuana every chance I get.", Author = "Allen Ginsberg" },
            new Quote { Id = "https://en.wikiquote.org/wiki/Allen_Ginsberg", Text = "The CIA and the Mafia are in cahoots.", Author = "Allen Ginsberg" },
            new Quote { Id = "https://en.wikiquote.org/wiki/Allen_Ginsberg", Text = "Nobody knows whether we were catalysts or invented something, or just the froth riding on a wave of its own. We were all three, I suppose.", Author = "Allen Ginsberg" },
        };
    }
}
